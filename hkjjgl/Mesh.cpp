...
void Mesh::ModifyTriangle() {
	glBindVertexArray(arrayObject);
	glBindBuffer(GL_ARRAY_BUFFER, bufferObject[VERTEX_BUFFER]);
	Vector3* ptr = (Vector3*)glMapBuffer(GL_ARRAY_BUFFER, GL_READ_WRITE);
	//m->numVertices = 4;
	if (ptr) {
		for (int i = 0; i < numVertices; i++) {
			ptr[i].x *= 0.999;
...
Mesh* Mesh::GenerateQuad() {
	...
	for (int i = 0; i < 4; ++i) {
		...
		m->normals[i] = Vector3(0.0f, 1.0f, 0.0f); //New!
...
Vector4  Mesh::GenerateTangent(int a, int b, int c) { 
	...
	Vector3  tangent;
	Vector3  binormal;
	// This is true length of Tangent times the portion in terms of u,v ---> equal to true BA and CA ; We get the tangent with invert op 
